window.onload = function() {
	...
	//hide
	$("#hide").click(function() {
		$("#control-bar").animate({left: $(window).width() }, 500);
		$("#menu-button").animate({left: $(window).width() -35 }, 500);
		hidden = true;
	});
	//show
	$("#menu-button").click(function() {
		$("#control-bar").animate({left: 0 }, 500);
		$("#menu-button").animate({left: -25 }, 500);
		hidden = false;
	...
	$(window).resize(function() {
		if (hidden == true){
			...
			$("#menu-button").css('left', $(window).width() -35);
		}
	});
	...
	if (theCanvas && theCanvas.getContext) {
		...
		if (ctx) {
			...
			var layer4 = setInterval(function(){ //(Math.random()*radius) + offset);
				...
				for (var i=0;i<=5 *lineQuantityInput;i++){ //for loop adds more strokes per iteration
			...
			setTimeout(function(){	
				var layer3 = setInterval(function(){
					for (var i=0;i<=5*lineQuantityInput;i++){
						//set random stroke width range
